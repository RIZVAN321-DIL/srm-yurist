...
def generate_csrf_token() -> str: return jwt.encode({"csrf": os.urandom(16).hex(), "exp": datetime.utcnow() + timedelta(hours=8)}, settings.CSRF_SECRET, algorithm="HS256")
...
async def create_case(data: CaseCreate, session=Depends(get_session), user=Depends(get_current_user)):
    ...
    if data.statute_deadline:
        from datetime import datetime as dt; statute = dt.fromisoformat(data.statute_deadline)
...
async def update_case(case_id: int, data: CaseUpdate, session=Depends(get_session), user=Depends(get_current_user)):
    ...
    if data.statute_deadline:
        from datetime import datetime as dt; updates["statute_deadline"] = dt.fromisoformat(data.statute_deadline)
...
async def create_stage(data: StageCreate, session=Depends(get_session), user=Depends(get_current_user)):
    deadline = None
    if data.deadline:
        from datetime import datetime as dt; deadline = dt.fromisoformat(data.deadline)
...
async def update_stage(stage_id: int, data: StageUpdate, session=Depends(get_session), user=Depends(get_current_user)):
    ...
    if data.deadline:
        from datetime import datetime as dt; updates["deadline"] = dt.fromisoformat(data.deadline)
...
echo ""
echo "=============================================="
echo "  CRM v6.3 FINAL — READY TO DEPLOY"
